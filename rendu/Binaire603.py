...
class Binaire603(list):  # Voir Object ?
    ...
    dbin = dict()
    for k in range(256):
        # Dictionnaire faisant correspondre un caractère à un octet
        dbin[lchr[k]] = k
    ...
    def __init__(self, param):
        ...
        if isinstance(param, str):
            ...
            lb = []
            for b in param:
                assert Binaire603.estOctet(
                    b), "Les éléments d'un Binaire603 doivent convertible en octets"
                lb.append(int(b))
            super().__init__(lb)
    ...
    def sauvegardeDansFichier(self, nomFic, verbose=False):
        ...
        if verbose:
            print(f"lecture du fichier :{nomFic}")
    ...
    def bin603DepuisFichier(nomFic, verbose=False):
        ...
        if nomFic[-4:].upper() == ".TXT":
            ...
            with open(nomFic, "rb") as f:
                data = f.read()
                if verbose:
                    print(f"data est de type : {type(data)}")
                b = Binaire603(data)
                if verbose:
                    print(f"data : {b}")
            return b
    ...
    def demoEntropie():
        for ext in ["txt", "odt", "pdf", "py", "odp", "zip", "jpg", "png", "pcx", "bmp"]:
            ...
            print(
                f"L'entropie du fichier {'Exemple.'+ext} est de : {Binaire603.bin603DepuisFichier('Exemple.'+ext).entropie():1.3f}")

        for k in range(5):
            # Bug de clé trop longue pour taille =100000
            monBin = Binaire603.exBin603(num=k, taille=2000)
            print("Bin:", repr(monBin))
            monBin.affFreq()

    def demoLivres():
        for nf in ["Les Miserables", "Notre Dame de Paris", "L Education sentimentale", "Guerre et Paix"]:
            lb = Binaire603.bin603DepuisFichier(
                os.listdir()[0]+"/txt/"+nf+".TXT")
            lb.afficheHistogrammeDesFrequences(
                titre="Fréquence des lettres dans "+nf)
