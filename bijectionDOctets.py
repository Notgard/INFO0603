...
class fAffine(fBijOctetsCA):
     ...
     def valInv(self,octetC):
         ...
         123
         """
         am1=arithmetiqueDansZEtd.ElementDeZnZ(self.a,256).inverse().val
...
class fPermut(object):
    ...
    def f(num,val):
        ...
        lp=fPermut.lp[num]
        lx=[val%2,(val//2)%2,(val//4)%2,(val//8)%2] #substitution
        return lx[lp[0]]+2*lx[lp[1]]+4*lx[lp[2]]+8*lx[lp[3]] #permutation
...
class fDecalage(fBijOctetsCA):
     def __init__(self, decalage=10):
        self.decalage = decalage

     def __repr__(self):
         return f"fDecalage({self.decalage})"

     def __call__(self,octet):
         return arithmetiqueDansZEtd.ElementDeZnZ(octet+self.decalage, 256).val

     def valInv(self,octetC):
         """Renvoie l'antécédent de octetC
         >>> fDecalage().valInv(fDecalage()(7))
         7
         """
         return arithmetiqueDansZEtd.ElementDeZnZ(octetC-self.decalage, 256).val
