...
class PolF2(object):
	...
	def __init__(self, x):
		...
		if isinstance(x, list):
			...
			for y in x:
				if (isinstance(y, int)):
					self.data.insert(0, ElementDeZnZ(y, 2))
				elif (isinstance(y, ElementDeZnZ)):
		...
		# Si x est un entier
			...
			self.data = [ElementDeZnZ(int(y), 2) for y in binStr[::-1]]
	...
	def distanceHamming(self, other):
		...
		# Sinon, on ajoute des 0 en fin de liste à celui qui a le plus petit degré
			...
			if difference > 0:
				...
				otherWithZeros = other.data + [ElementDeZnZ(0,2)] * difference
			# Sinon, on ajoute des 0 en fin de liste à self
			else:
				selfWithZeros = self.data + [ElementDeZnZ(0,2)] * -difference
